# Lesson 7: Connect 4 v3 -- Pygame Edition
...
You might notice this code is all jammed into one big loop again -- no functions. That's on purpose! We cleaned things up with functions in Lesson 6, but now we're learning a completely new library (Pygame), so we're keeping it simple. We'll add functions back later.
