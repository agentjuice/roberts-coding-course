...
The `try`/`except` handles the first time you play -- there's no file yet, so we just return an empty list. Remember that pattern from Lesson 19? Same idea. The **`with` statement** automatically closes the file when we're done. Always use `with` for files!
